...
public partial class Familias_InsertarFamilia : System.Web.UI.Page
{
    ...
    private void checkStatus(int id)
    {
        ...
        DataSet ds = ABMCProductReview.retrieveProductReviewByProductId(id);
    ...
    {
        if (!ABMCProductReview.blockProduct(pId))
        {
    ...
    private Product cargarDatosBase(int id)
    {
        ...
        if (ds.Tables[0].Rows.Count != 0)
        {
            ...
            this.TextBoxFemeninoAdulto.Text = row[5].ToString();
            this.TextBoxMasculinoAdulto.Text = row[6].ToString();
            this.TextBoxFemeninoAdultoMayor.Text = row[7].ToString();
            this.TextBoxMasculinoAdultoMayor.Text = row[8].ToString();
        }
    ...
    private void guardar()
    {
        ...
        Boolean state = ABMCProductReview.saveReviewOfProduct(review);
        if (!state)
        {
            this.redirectToPage(review.ProductId);
        }

        ABMCProductReview.deleteBlock(id);
